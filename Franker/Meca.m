classdef Meca < RobotBaseClass
    properties(Access =public)   
        plyFileNameStem = 'Meca';
    end
    methods (Access = public) 
        function self = Meca(baseTr)
			self.CreateModel();
            if nargin < 1
                baseTr = eye(4);
            end
            ...
            % axis([, 0.25, -0.25, 0.25, 0, 1.5]);
        end
        function CreateModel(self)       
            link(1) = Link('d', 0.000, 'a', 0.0000, 'alpha', 0,'offset',0);
            link(2) = Link('d', 0.000, 'a', 0.0000, 'alpha', pi/2, 'offset',0);
            % link(3) = Link('d', 0.316, 'a', 0.0000, 'alpha', pi/2, 'offset',0);
            % link(4) = Link('d', 0.000, 'a', 0.0800, 'alpha', -pi/2, 'offset',0);
