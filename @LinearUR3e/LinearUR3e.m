classdef LinearUR3e < RobotBaseClass
    %% LinearUR3e on a non-standard linear rail created by a student and modified by me
...
%% Create the robot model
        function CreateModel(self)   
            ...
            % %ORIGINAL LINK LIMITS (-360 360) FOR ALL
            ...
            % Incorporate joint limits (ORIGINAL)
            ...
            % 
            % % Incorporate MODIFIED joint limits
            % link(1).qlim = [-0.8 -0.01];
            % link(2).qlim = [-360 360]*pi/180;
            % link(3).qlim = [0 70]*pi/180;
            % link(4).qlim = [89 91]*pi/180;
            % link(5).qlim = [-44 46]*pi/180;
            % link(6).qlim = [269 271]*pi/180;
            % link(7).qlim = [-360 360]*pi/180;
